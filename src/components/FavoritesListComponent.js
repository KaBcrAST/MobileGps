...
// Modifié pour afficher uniquement la liste des favoris
...
const FavoritesListComponent = ({ onSelectRoute, onClose }) => {
  ...
  const handleSelectFavorite = async (favorite) => {
    try {
      ...
      // Obtenir l'itinéraire pour la navigation directe en utilisant le service de navigation
  ...
  // Modifier le renderItem pour utiliser handleSelectFavorite au lieu de onSelectRoute directement
...
const styles = StyleSheet.create({
  ...
  favoriteItem: { flexDirection: 'row', alignItems: 'center', padding: 15, borderBottomWidth: 1, borderBottomColor: '#eee' },
  favoriteIcon: { marginRight: 12 },
  favoriteContent: { flex: 1 },
  favoriteName: { fontSize: 16, fontWeight: 'bold', color: '#333', marginBottom: 4 },
  addressRow: { flexDirection: 'row', alignItems: 'center' },
  tinyIcon: { marginRight: 5 },
  addressText: { fontSize: 14, color: '#666', flex: 1 },
  navigateIcon: { marginLeft: 10, opacity: 0.7 },
  centerContent: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 },
  loadingText: { marginTop: 10, color: '#666', fontSize: 16 },
  errorText: { marginTop: 10, color: '#FF4A4A', fontSize: 16, textAlign: 'center' },
  retryButton: { marginTop: 20, paddingVertical: 10, paddingHorizontal: 20, backgroundColor: '#4A3AFF', borderRadius: 5 },
  retryText: { color: '#fff', fontWeight: 'bold' },
  emptyText: { marginTop: 10, fontSize: 18, fontWeight: 'bold', color: '#666' },
  emptySubtext: { marginTop: 5, fontSize: 14, color: '#95a5a6', textAlign: 'center' },
  navigationOverlay: {
    position: 'absolute',
    top: 0,
    left: 0,
